...
const UserTable: React.FC = () => {
  ...
  const apiUrlBase = import.meta.env.VITE_API_BASE_URL || "http://localhost:8000/api";
  ...
  const handleEditClick = () => {
    if (selectedUserId !== null) {
      navigate(`/editar-usuario/${selectedUserId}`);
    }
    handleMenuClose();
  };
    ...
    <Paper sx={{ padding: 2 }}>
      ...
      <TableContainer component={Paper}>
        <Table sx={{ minWidth: 650 }} aria-label="simple table">
          ...
          <TableBody>
            {sortedUsers.slice((page - 1) * rowsPerPage, page * rowsPerPage).map((user) => (
